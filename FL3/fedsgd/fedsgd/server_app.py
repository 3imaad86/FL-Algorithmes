...
La strategie FedAvg native de Flower convient : moyenner les poids de
clients qui ont fait UN pas SGD est equivalent a moyenner les gradients.
"""
...
from fedsgd.metrics_utils import (
...
from fedsgd.task import Net, model_size_bytes
...
def main(grid: Grid, context: Context) -> None:
    ...
    print(f"[config] algo=FedSGD partitioning={partitioning}{extra} "
    ...
    def agg_train(records, wk):
        ...
        for rec in recs:
            ...
            epochs_list.append(float(mr.get("epochs_used", 1.0)))
            ...
            clients_detail.append({
                ...
                "epochs": float(mr.get("epochs_used", 1.0)),
    ...
    dst = resolve_dst_results_dir("fedsgd")
